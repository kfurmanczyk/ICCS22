...
#Lassojoint
library(glmnet)
lassojoint <- function(x_train, y_train, x_test, nfolds = NULL, lambda = "lambda.min"){
  ...
  est = exp(as.matrix(x_test[,nosnik1])%*%betas13 + betas03) / (1 + exp(as.matrix(x_test[,nosnik1])%*%betas13 + betas03))
...
lassoCDMM <- function(x_train, y_train, x_test, nfolds = NULL, lambda = "lambda.min"){
  ...
  est = logistic_cdmm(x_train = x11, y_train = s11, x_test = xtt)
